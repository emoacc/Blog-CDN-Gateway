let urls = [
	'https://uomi.cc#悠米商城丨地址一',
	'https://hk.uomi.cc#悠米商城丨地址二',
	'https://jp.uomi.cc#悠米商城丨地址三'
];
...
export default {
	async fetch(request, env) {
		...
		const 网站图标 = env.ICO || 'https://uomi.cc/favicon.svg';
		const 网站头像 = env.PNG || 'https://uomi.cc/favicon.svg';
		const 网页标题 = env.TITLE || '悠米商城丨永久发布页';
		const 站点名称 = env.NAME || '悠米商城丨永久发布页';
		if (url.pathname.toLowerCase() == '/ads.txt') {
			...
			// 先测速，不加载背景图片
			let img = 'https://api.bimg.cc/today?w=1920&h=1080&mkt=zh-CN'; // 默认图片
			if (env.IMG) {
